...
public class UserController {
    ...
    public String listUsers(@RequestParam( value = "page" , defaultValue = "0") int page, @RequestParam(value = "size" , defaultValue = "10") int size, Model model) {
        ...
        List<String> permissions = permissionRepository.findAll().stream().map( p -> p.getName()).toList();
        model.addAttribute("permissions", permissions);
        return "admin/user-management";
    }
    // create user
    @PostMapping("/create")
    public String createUser(@ModelAttribute UserCreationRequest request,
                             RedirectAttributes redirectAttributes) {
        try {
            userService.createUser(request);
            redirectAttributes.addFlashAttribute("success", "Create user success");
        } catch (ApiException e) {
            redirectAttributes.addFlashAttribute("error", e.getErrorCode() != null ? e.getErrorCode().getMessage() : e.getMessage());
        }
    ...
    public String updateUser(@PathVariable String username,
        ...
        try {
            ...
            redirectAttributes.addFlashAttribute("error",
                                                 e.getErrorCode() != null ? e.getErrorCode().getMessage() : e.getMessage());
        }
        return "redirect:/admin/users";
    }
    ...
    public String findUserByUsername(
        ...
        Page<UserResponse> users =
                (search != null && !search.isBlank())
                        ? userService.searchUser(search, pageable)
